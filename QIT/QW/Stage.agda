...
module WithZ {ℓA} (ZA : ZAlg.Algebra ℓA) where
  ...
  abstract
    ...
    ssup₀ : ∀ a μ (f : ∀ i → S₀ (μ i)) → S₀ (Z.sup (ιˢ a , μ))
    ...
    ≈scong : ∀ a μ (f g : ∀ i → S₀ (μ i))
          → (r : ∀ i → μ i ⊢ f i ≈ᵇ g i)
          → Z.sup (ιˢ a , μ) ⊢ ssup₀ a μ f ≈ᵇ ssup₀ a μ g
    ...
    S̃ : Z → Setoid (ℓS ⊔ ℓP ⊔ ℓA) (ℓS ⊔ ℓP ⊔ ℓE ⊔ ℓV)
    S̃ α = record
      { Carrier = S₀ α
      ; _≈_ = λ ŝ t̂ → ŝ ≈ˢ t̂
    ...
    --         → hom p (D̃ α ⊢[ s ]) ≡ D̃ β ⊢[ dweaken₀ (unbox p) s ]
    -- hom-beta {α} {β} (box α≤β) s =
    --   Qα.rec-beta (λ (s : S₀ α) → Qβ.[ dweaken₀ α≤β s ]) Qβ.≈[_] s
    --   where
