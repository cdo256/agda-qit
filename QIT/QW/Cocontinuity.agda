...
module WithZ {ℓA} (ZA : ZAlg.Algebra ℓA) where
  ...
  module _ (depth-preserving : ∀ (s t : T) → s ≈ᵗ t → ιᶻ s ≡ ιᶻ t) where
    module PreservationByPowers
      ...
      rankD-cong : {s t : T} → s ≈ᵗ t → ιᶻ s ≡ ιᶻ t
      rankD-cong {s} {t} p = depth-preserving s t p
      ...
      module Plift≈Helper {α} where
        ...
        exactify {γ} {s , s≤γ} {t , t≤γ} p =
          ≈ttrans p (q (≡.sym (rankD-cong p)))
          -- Goal: t S.≈ᵗ fst (subst D₀ _ (plift (t , t≤γ)))
          where
          q : (dp : ιᶻ t ≡ ιᶻ s) → t ≈ᵗ subst D₀ dp (t , ≤refl _) .fst
          q dp = ≡.substp (t ≈ᵗ_) (≡.cong fst r) ≈trefl
            where r : (t , ≡.substp (t ≤ᵀ_) dp (≤refl _)) ≡ subst D₀ dp (t , ≤refl _)
                  r = ≡.sym (≡.subst-ΣP (λ α t → t ≤ᵀ α) dp (t , ≤refl _))
    --     exactify (≈pcong a μ f₁ g r) =
    --       castˡ (D̃ _) (plift-psup a μ f₁) (castʳ (D̃ _) rhs≈ base)
    --       where
    ...
    --     exactify (≈psat e ϕ l≤α r≤α) = castʳ (D̃ _) rhs≈ base
    --       where
    ...
    --       castˡ (D̃ _) {z = subst D₀ dp (plift t̂)} lhs≈ (transport≈ {!D̃ _!} dp (≈psym (exactify p)))
    --       where
    ...
    --     qs : s ~ᵀ s'
    --     qs = depth-preserving α s s' rs

    --     qt : t' ~ᵀ t
    --     qt = depth-preserving β t' t rt
