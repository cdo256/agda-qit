...
--   module ColimF∘D = SQ (Colim (F ∘ꟳ D))
...
--   --         → D̃ γ [ (s , ps) ≈ (t , pt) ]
--   --         → D̃ γ [ (s , ps) ≈ (t , qt) ]
--   --       cast-rhs {pt = pt} {qt = qt} p = ≈dtrans p (same-stage pt qt)
...
--   --       exactify (≈dcong a μ f₁ g r) = castˡ (plift-psup a μ f₁) (castʳ rhs≈ base)
--   --         where
...
--   --         base = ≈dcong a μ' f' g' (λ i → exactify (r i))
--   --         dp : rankD₀ (psup a μ f₁) ≡ rankD₀ (psup a μ g)
--   --         dp = rankD-cong (≈dcong a μ f₁ g r)
...
--   --       exactify (≈dsat e ϕ l≤α r≤α) = castʳ rhs≈ base
--   --         where
--   --         dp : rankD₀ (lhs' e ϕ , l≤α) ≡ rankD₀ (rhs' e ϕ , r≤α)
--   --         dp = rankD-cong (≈dsat e ϕ l≤α r≤α)
--   --         base : D̃ (rankD₀ (lhs' e ϕ , l≤α)) [ plift (lhs' e ϕ , l≤α) ≈ (rhs' e ϕ , ≡.substp (rhs' e ϕ ≤ᵀ_) (≡.sym dp) (≤refl _)) ]
--   --         base = ≈dsat e ϕ (≤refl _) (≡.substp (rhs' e ϕ ≤ᵀ_) (≡.sym dp) (≤refl _))
...
--   --       exactify {ŝ = ŝ} {t̂ = t̂} ≈drefl =
--   --         ≡→≈ (D̃ (rankD₀ ŝ)) (≡.sym (≡.subst-refl (plift ŝ)))
--   --       exactify {ŝ = ŝ} {t̂ = t̂} (≈dsym p) =
--   --         castˡ {z = subst D₀ dp (plift t̂)} lhs≈ (transport≈ dp (≈dsym (exactify p)))
--   --         where
--   --         dp : rankD₀ t̂ ≡ rankD₀ ŝ
--   --         dp = rankD-cong p
--   --         lhs≈ : subst D₀ dp (subst D₀ (≡.sym dp) (plift ŝ)) ≡ plift ŝ
--   --         lhs≈ = ≡.subst-inv D₀ (≡.sym dp)
--   --       exactify {ŝ = ŝ} {t̂ = û} (≈dtrans {ŝ = ŝ} {t̂ = t̂} {û = û} p q) = castʳ rhs≈ (≈dtrans (exactify p) mid)
--   --         where
...
--   --         rhs≈ : subst D₀ (≡.sym dp) (subst D₀ (≡.sym dq) (plift û)) ≡ subst D₀ (≡.sym (rankD-cong (≈dtrans p q))) (plift û)
--   --         rhs≈ = ≡.subst-subst D₀ (≡.sym dq) (≡.sym dp) (plift û)
--   --       exactify (≈dweaken α≤β p) = exactify p
