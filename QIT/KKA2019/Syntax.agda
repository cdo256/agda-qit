...
interleaved mutual
  ...
  _↑ {Γ} {A = A} σ = (σ ∘ wk) , ≈t-subst (≈T-sym ([∘] A σ wk)) vz
  ...
  data _ where
    ...
    ≈t-subst : A ≈ᵀ B → Tm Γ A → Tm Γ B
    -- ≈t-subst : A ≈ᵀ B → (t : Tm Γ A) (u : Tm Γ B) → A ≈ᵗ B → Tm Γ A → Tm Γ B
...
module Example1 where
  ...
  I a = λⁱ a vz
  K : (a : Tm Γ U) (b : Tm Γ U) → Tm Γ (Πⁱ a (Πⁱ (≈t-subst (U[] wk) (b [ wk ]ᵗ)) (El a [ π₁ wk ]ᵀ)))
  K a b = λⁱ a (λⁱ (≈t-subst _ (b [ wk ]ᵗ)) (π₂ wk))
