...
invoked can in fact be carried out fully constructively. This provides evidence
that a wider class of infinitary QITs can be constructed from quotient types
only.

\section{Mobiles}

We study \emph{mobiles}, which have just two point constructors
...
proofs available within a bounded depth. The desired QIT is obtained as the
...
required isomorphism, i.e. an isomorphism $F(\mathrm{colim} D) \cong \mathrm{colim}(F \circ D)$.
...
This suggests that there may be a significant class of QITs which were previously
