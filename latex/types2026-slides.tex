...
% Fix duplicate PDF destination warnings (NavigationN, page.N) caused by
% pgfpages creating two logical pages per frame.  The patch suppresses the
% hyperref anchor hooks on the notes logical page so each named destination
% is only emitted once.  Requires the xpatch package.
\usepackage{xpatch}
\makeatletter
\xpatchcmd{\beamer@outsideframenote}{\pgfpagescurrentpagewillbelogicalpage}{%
  \renewcommand*{\HyPL@EveryPage}{\ltx@empty}%
  \renewcommand*{\Hy@EveryPageAnchor}{\ltx@empty}%
  \pgfpagescurrentpagewillbelogicalpage}%
  {}{\errmessage{Patch beamer@outsideframenote for note page FAILED}}%
\makeatother
...
\author{Christina O'Donnell\texorpdfstring{\\[4pt]}{, }%
  \footnotesize Supervised by Prof. Thorsten Altenkirch}
...
\begin{frame}{Question}
  \begin{center}
    \Large
    When does quotienting an inductive type really give the
    quotient inductive type we want?
  \end{center}
  \note{%
    Timing: 0:20.

    This talk is about one basic question.

    When do quotients and inductive types give rise to quotient inductive types?

    In the infinitary branching case, the standard constructions seem
    to rely on a weak form of choice.

    I want to explore where that need for choice comes from, by
    working through a specific motivating example.
  }
\end{frame}
...
\begin{frame}{Outline}
  \begin{enumerate}
    \item A simple example: mobiles
    \item Why the obvious construction fails
    \item Where weak choice enters
    \item A positive case
    \item Why the general problem is still open
  \end{enumerate}
  \note{%
    Timing: 0:40 (cumulative: 1:00).
  
    Here is the plan.
  
    First I will introduce the motivating structure which we call
    ``mobiles'' and explain the lifting problem.
  
    Then I will say where choice enters in the size indexed stage
    construction.
  
    After that I will explain the key observation for mobiles, namely that
    permutation does not change depth.
  
    Then I will sketch the proof that depth preserving equations gives
    us a section on the colimit morphism, say what extra ordinal
    structure we assume, and sketch why that is enough in this case.
  
    Finally I will state a working conjecture on the provability of
    mobiles from quotients and W-types alone.
  }
\end{frame}
...
\begin{frame}{The story so far}
  \begin{itemize}
    ...
    \item Finitary QIITs admit a universal reduction via the theory of signatures.
    \item Large and infinitary QIITs have a general syntax/semantics, but are
          harder to construct (Kov\'acs--Kaposi 2020).
    \item Infinitary QITs are known difficult (Blass 1983, Lumsdaine--Shulman 2020).
    \item We want to investigate where the boundary between the
          constructible and non-constructible QITs arises.
  \end{itemize}
  \note{%
    Timing: 0:45 (cumulative: 1:45).

    This is the background I am starting from.

    Quotient inductive types are the set-level version of higher inductive types.

    In the finitary case, Kaposi, Kov\'acs, and Altenkirch reduce general
    QIITs to one universal QIIT, namely the theory of signatures.

    So that gives a powerful reduction principle, but it does not by itself
    construct that universal object.

    In the large and infinitary case, there is also a general syntax and
    semantics, but actual constructions are harder.
...
\begin{frame}{A motivating example}
  ...
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      ...
      \begin{center}
      \begin{tikzpicture}[scale=0.55, every node/.style={inner sep=1.5pt}]
    ...
    \begin{column}{0.42\textwidth}
      \includegraphics[width=\linewidth]{fish-mobile}
    \end{column}
  \end{columns}
  \note{%
    Timing: 1:00 (cumulative: 2:45).
...
\begin{frame}{Extensional plump ordinals}
  ...
  \]
  \note{%
    Timing: 1:15 (cumulative: 8:45).

    At this point I need to say clearly what extra structure I am assuming.

    We are not assuming full choice.

    What we postulate is an extensional plump ordinal object \(Z\), equipped
...
\begin{frame}{Proof sketch}
  For mobiles, permutation equations are \textbf{depth-preserving}.
  ...
  \qquad
  \mathrm{rank}(node f) = \mathrm{sup}_i (\mathrm{rank}_0 (f_i)
  \]
  respects the quotient relation at each stage, and descends to maps
  \[
    \mathrm{rank}_\alpha : D_\alpha \to Z.
  \]
  ...
  This coherent stage assignment gives the \textbf{section} needed to define the
  constructors on the colimit.

  \note{%
    Timing: 1:45 (cumulative: 11:40).

    Here is the proof idea.

    First, for mobiles, permutation does not change depth, so the raw stage
    assignment rank0 from the raw tree W to the plump ordinal Z
    respects the quotient relation inside each stage.

    That means it descends to maps rank alpha from the stage at some
    ordinal alpha to alpha to Z.

    Second, these descended rank maps are compatible with the
    connecting inclusion morphisms of the diagram, so they induce a
    single map rank from the colimit of D, to Z.
...
\begin{frame}{Result for mobiles}
  ...
  So the quotient inductive type of mobiles is obtained from the stagewise
  argument relative to an extensional plump ordinal basis.

  \medskip

  This argument has been partially formalised in Agda.
  \note{%
    Timing: 0:30 (cumulative: 12:10).
    ...
    So in this case, the full weak-choice argument is not needed in the
    same way as in the general FPS proof.
...
\begin{frame}{Can we generalise beyond mobiles?}
  ...
  \note{%
    Timing: 2:00 (cumulative: 14:10).

    Depth preservation is a purely syntactic property: you can read it
    off directly from the equations.

    The obvious next question is whether one can weaken it.  The most
    natural weakenings are bounded depth shift, or locality, where both
    sides stay below some fixed depth bound.

    But this cannot be the whole story.  Even if such a syntactic bound
    exists, a large enough extensional ordinal may not be constructively
    available — and that is exactly the choice-like strength we are
    trying to avoid.

    Also, bounded syntax alone does not control ordinal strength in
    general: Blass's example (later reframed by Lumsdaine and Shulman)
    shows that apparently controlled equations can still generate large
    ordinal strength.

    So depth preservation gives a genuine positive case, but the right
    general criterion remains open.
  }
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  the quotient inductive type of mobiles is not derivable from
  UIP + funExt + Prop + quotients + \(W\)-types alone,
  without an additional ordinal principle.

  \note{%
    Timing: 0:45 (cumulative: 14:55).
    ...
    Extensional plump ordinals provide the ordinal basis for the mobile
    case.

    Our current working conjecture is that this extra ordinal basis is not
    just an artefact of the proof.

    In other words, mobiles are not derivable from UIP, function
    extensionality, Prop, quotients, and W-types alone.

    Proving or refuting that is the next step.
  }
\end{frame}
...
\begin{frame}
  ...
  Links:
  \begin{itemize}
    \item Source: \url{https://github.com/cdo256/agda-qit}
    \item Christina's website: \url{https://octocurious.com}
    \item Thorsten's podcast: \url{https://www.youtube.com/@aboutlogic}
  \end{itemizeSlides}
  \vfill
  \note{%
    Timing: 0:10 (cumulative: 15:05).
  }
\end{frame}
