...
Unlike the FPS construction, which expands on free F-algebras as the we use the depth of terms directly as their
...
$\alpha \le\ge \beta$ for mutual boundedness.
...
\[
	\hat s = (s, s \le \alpha)
\]
where $s:T$ and $s\le\alpha$ is a proof that $\iota(s) \le \alpha$.
We write
\[
	\alpha \vdash \hat s \approx^b \hat t
\]
for the stagewise equality relation used to form the quotient at stage
$\alpha$.
...
\begin{definition}[Depth-preserving QIT]
	...
	\[
		...
		\iota^z(\hat s.\mathsf{fst}) \le\ge \iota^z(\hat t.\mathsf{fst}).
	\]
\end{definition}
...
\begin{proof}[Proof sketch]
	The only nontrivial generating equation is permutation at a node:
	$\mathsf{node}(f) = \mathsf{node}(f \circ \varphi)$. This equation
	preserves the outer constructor and does not inspect subtrees, so it
	preserves depth. Congruence and the equivalence rules for equality also
	preserve depth. Hence any derivation of $\approx^b$ relates trees of the
	same depth.
\end{proof}

\subsection{Tight equality without explicit bounds}

Depth preservation allows us to replace bounded equality $\approx^b$ by a
``tight'' equality on the underlying trees that reconstructs bounds from
their depths.

Define mutual boundedness on trees by
\[
	s \le\ge^T t \;\;:\!\!\equiv\;\; \iota^z(s) \le\ge \iota^z(t).
\]

\begin{definition}[Tight equality]
	For $s,t:T$, define $s \approx^s t$ to consist of:
	\begin{enumerate}
		\item a proof that $s \le\ge^T t$, and
		\item a bounded equality proof
		      \[
			      \iota^z(s) \vdash (s,\_) \approx^b (t,\_),
		      \]
...
\begin{lemma}[Tightening]
	...
	If $\alpha \vdash \hat s \approx^b \hat t$ in $D_\alpha$, then
	$\hat s.\mathsf{fst} \approx^s \hat t.\mathsf{fst}$.
\end{lemma}
...
\subsection{Locality and cocontinuity}

Let $D \coloneqq \mathrm{colim}_\alpha D_\alpha$ be the FPS colimit. In
general, showing that the polynomial functor $F$ preserves this colimit
requires bounding families of subtrees uniformly, which is where FPS use
WISC \cite{fiore2022-quotients-inductive-types,vandenberg2012-wisc-axiom}.

For depth-preserving signatures, the situation simplifies: any equality
between terms can be witnessed at a stage determined by the depths of
those terms alone. The size index no longer carries essential
information; it can be reconstructed from the term shapes.

\begin{theorem}[Constructive cocontinuity for depth-preserving QITs]
	\label{thm:constructive-cocontinuity}
	Let a QW signature be depth-preserving. Then the associated polynomial
	functor $F$ preserves the colimit of the FPS size-indexed diagram
	constructively, i.e.\ without using WISC.
\end{theorem}

\begin{proof}[Proof idea]
	Using tight equality, constructor formation can be defined directly on
	colimit representatives by structural recursion on trees, since all
	necessary bounds can be reconstructed from depths. No global choice of
	bounding covers is required.
\end{proof}

\begin{corollary}[Initial algebra without WISC]
	\label{cor:initial-algebra-local}
	For a depth-preserving QW signature, the FPS construction yields an
	initial $F$-algebra satisfying the equations, without assuming WISC.
\end{corollary}

In the next section we apply this result to the mobile signature,
completing the construction of mobiles as a quotient inductive type in
predicative type theory with quotients alone.
