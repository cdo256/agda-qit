...
## Mobile Diagram Construction

### Mobile Elements

For each plump ordinal $\alpha : Z$, define the type $P_0(\alpha)$ of mobile elements at stage $\alpha$ inductively:
...
The key insight is that mobile elements are not simply bounded trees, but structured terms that can be:
- **Leaves** at any node stage (indexed by functions $\beta : I \to Z$)
- **Nodes** with recursively defined mobile children at smaller ordinals
- **Weakened** from smaller to larger stages via the plump ordering

### Mobile Equivalence

Define the mobile equivalence relation $\approx^P$ on mobile elements by the generators:

- **Leaf equivalence**: $\mathsf{leaf}(\beta) \approx^P \mathsf{leaf}(\gamma)$ for any $\beta, \gamma : I \to Z$
- **Node congruence**: $\mathsf{node}(\beta, f) \approx^P \mathsf{node}(\gamma, g)$ if $\forall i : I. f(i) \approx^P g(i)$
- **Permutation symmetry**: $\mathsf{node}(\beta, f) \approx^P \mathsf{node}(\beta \circ \pi, f \circ \pi)$ for any bijection $\pi : I \leftrightarrow I$
- **Weakening transparency**: $s \approx^P \mathsf{weaken}(\alpha, \gamma, p, s)$ for any $p : \alpha \le \gamma$
- **Symmetry and transitivity**: $\approx^P$ is symmetric and transitive

This gives mobile equivalence the crucial **permutation symmetry** that distinguishes mobile categories from ordinary QIT signatures.
...
This construction ensures that each stage forms a proper setoid with reflexivity, symmetry, and transitivity of the mobile equivalence relation.

### The Mobile Diagram

The mobile diagram structure arises from the plump ordering on ordinals. For any $p : \alpha \le \beta$ in the plump order, we obtain a setoid homomorphism:

$$P(p) : P(\alpha) \to P(\beta)$$

defined by $P(p)(s) := \mathsf{weaken}(\alpha, \beta, p, s)$ with congruence established by the weakening transparency of mobile equivalence.

This yields the mobile diagram:
$$P : (Z, \le) \longrightarrow \mathbf{Setoid}$$

satisfying the functorial properties:
- **Identity**: $P(\mathsf{refl}) \approx_h \mathsf{id}$
- **Composition**: $P(q \circ p) \approx_h P(q) \circ P(p)$

The mobile diagram provides the foundation for constructing colimits that respect both the ordinal structure and the crucial permutation symmetries needed for mobile category theory.

### The Mobile Colimit

The colimit of the mobile diagram $P : (Z, \le) \to \mathbf{Setoid}$ is constructed as follows:

**Carrier**: The colimit carrier is the dependent sum:
$$\text{Colim}_0 = \sum_{\alpha : Z} |P(\alpha)|$$

**Equivalence Relation**: Define the colimit equivalence relation $\approx^l$ on $\text{Colim}_0$ inductively by:

- **Stage equivalence**: $(\alpha, x) \approx^l (\alpha, x')$ if $P(\alpha)[x \approx^P x']$
- **Step equivalence**: $(\alpha, x) \approx^l (\beta, P(p)(x))$ for any $p : \alpha \le \beta$
- **Symmetry**: $s \approx^l t \implies t \approx^l s$
- **Transitivity**: $s \approx^l t \land t \approx^l u \implies s \approx^l u$

**The Colimit Setoid**: 
$$\text{Colim}(P) := (\text{Colim}_0, \approx^l)$$

**Cocone Structure**: The canonical injections are:
$$\iota_\alpha : P(\alpha) \to \text{Colim}(P)$$
$$\iota_\alpha(x) := (\alpha, x)$$

These satisfy the cocone property: for any $p : \alpha \le \beta$, we have $\iota_\beta \circ P(p) \approx_h \iota_\alpha$.

**Universal Property**: $\text{Colim}(P)$ satisfies the universal property that for any other cocone with apex $A$ and injections $\{\kappa_\alpha : P(\alpha) \to A\}$, there exists a unique setoid homomorphism $h : \text{Colim}(P) \to A$ making all triangles commute.
